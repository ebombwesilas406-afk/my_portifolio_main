...
type ModuleKey = "MODULE_01_DEV" | "MODULE_02_TOOLS" | "MODULE_03_ENGINEERING";

const moduleSpec: Record<ModuleKey, readonly string[]> = {
  MODULE_01_DEV: ["Python", "JavaScript", "SQL"],
  MODULE_02_TOOLS: ["Django", "React", "Git"],
  MODULE_03_ENGINEERING: ["Backend Logic", "Database Design", "System Design"]
};
...
function ModuleHeader({ label }: { label: ModuleKey }) {
...
export default function Skills({ skills }: SkillsProps) {
  ...
  const modules = useMemo(() => {
    const pick = (name: string) => byName.get(name) ?? null;

    return {
      MODULE_01_DEV: moduleSpec.MODULE_01_DEV.map(pick).filter(Boolean) as Skill[],
      MODULE_02_TOOLS: moduleSpec.MODULE_02_TOOLS.map(pick).filter(Boolean) as Skill[],
      MODULE_03_ENGINEERING: moduleSpec.MODULE_03_ENGINEERING.map(pick).filter(Boolean) as Skill[]
    } as const;
  }, [byName]);

  const hoveredInModule = useMemo(() => {
    if (hoveredId === null) return null;
    for (const label of Object.keys(modules) as ModuleKey[]) {
      const match = modules[label].some((s) => s.id === hoveredId);
  ...
  return (
    <section id="skills" className="section-shell console-grid skills-circuit">
      ...
      {isEmpty ? (
        ...
        >
          <motion.div layout className="md:col-span-5">
            <ModuleShell label="MODULE_01_DEV" className="h-full">
              <div className="space-y-2">
                {modules.MODULE_01_DEV.map((skill) => (
                  <SkillRow
                    key={skill.id}
                    skill={skill}
                    hovered={hoveredId === skill.id}
                    dimmed={hoveredInModule === "MODULE_01_DEV" && hoveredId !== null && hoveredId !== skill.id}
          ...
          <motion.div layout className="md:col-span-7">
            <ModuleShell label="MODULE_02_TOOLS" className="h-full">
              <div className="grid gap-2 sm:grid-cols-2">
                {modules.MODULE_02_TOOLS.map((skill) => (
                  <SkillRow
                    key={skill.id}
                    skill={skill}
                    hovered={hoveredId === skill.id}
                    dimmed={hoveredInModule === "MODULE_02_TOOLS" && hoveredId !== null && hoveredId !== skill.id}
                    onHover={setHoveredId}
                  />
                ))}
              </div>
          ...
          <motion.div layout className="md:col-span-12">
            <ModuleShell label="MODULE_03_ENGINEERING">
              <div className="grid gap-2 md:grid-cols-2">
                {modules.MODULE_03_ENGINEERING.map((skill) => (
                  <SkillRow
                    key={skill.id}
                    skill={skill}
                    hovered={hoveredId === skill.id}
                    dimmed={
                      hoveredInModule === "MODULE_03_ENGINEERING" &&
                      hoveredId !== null &&
                      hoveredId !== skill.id
                    }
                    onHover={setHoveredId}
